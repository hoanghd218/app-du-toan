...
const INITIAL_CONFIG: HouseConfig = {
  ...
  packagePrices: { ...UNIT_PRICES } // Initialize with default prices
};
...
function App() {
  ...
  // Calculate results for all packages for comparison
  const comparisonResults = useMemo(() => {
    return {
      [PackageType.ROUGH]: calculateCost({ ...config, unitPrice: config.packagePrices[PackageType.ROUGH], packageType: PackageType.ROUGH }),
      [PackageType.FULL_AVERAGE]: calculateCost({ ...config, unitPrice: config.packagePrices[PackageType.FULL_AVERAGE], packageType: PackageType.FULL_AVERAGE }),
      [PackageType.FULL_GOOD]: calculateCost({ ...config, unitPrice: config.packagePrices[PackageType.FULL_GOOD], packageType: PackageType.FULL_GOOD }),
      [PackageType.FULL_PREMIUM]: calculateCost({ ...config, unitPrice: config.packagePrices[PackageType.FULL_PREMIUM], packageType: PackageType.FULL_PREMIUM }),
    };
  }, [config]);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 xl:col-span-8">
            <ResultsView result={result} comparisonResults={comparisonResults} currentPackage={config.packageType} />
          </div>
